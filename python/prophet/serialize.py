...
SIMPLE_ATTRIBUTES = [
    ...
    'y_scale', 'y_min', 'logistic_floor', 'country_holidays', 'component_modes'
]
...
def model_from_dict(model_dict):
    ...
    if 'y_min' not in model_dict:
        if model.history.shape[0] > 0:
            model.initialize_scales(True, model.history)
            model_dict['y_min'] = model.y_min
            model_dict['y_scale'] = model.y_scale
        else:
            raise ValueError(
                "No history data found in model_dict, please re-train the model with "
                "prophet >= 1.2.x"
            )
